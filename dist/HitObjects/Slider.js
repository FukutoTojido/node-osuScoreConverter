...
class Slider extends HitObject {
    ...
    constructor(rawData, idx) {
        ...
        this.beatStep =
            Beatmap.baseData.timingpoints.filter((line) => line.uninherited && line.time <= this.time).at(-1)?.beatLength ??
                Beatmap.baseData.timingpoints[0].beatLength;
    ...
    eval(inputIdx) {
        ...
        while (currentInput.time <= this.endTime) {
            ...
            const accountedPointAtT = ScoreConverter.mods.includes("HardRock")
            ...
            // if (this.time === 119613) console.log(currentInput.time);
            if (sliderParts[sliderPartIdx] && ScoreConverter.cursorInputData[internalInputIdx + 1].time > sliderParts[sliderPartIdx]?.time) {
                if (currentInput.time !== sliderParts[sliderPartIdx]?.time) {
                    const nextInput = ScoreConverter.cursorInputData[internalInputIdx + 1];
                    const estimatedInput = LinearEstimation(currentInput, nextInput, (sliderParts[sliderPartIdx].time - currentInput.time) / (nextInput.time - currentInput.time));
                    // Untrack slider if release keys / move out of slider follow circle
                    if (state === "TRACKING")
                        if (currentInput.inputArray.length === 0 || Fixed(Dist(estimatedInput, sliderParts[sliderPartIdx]) / (2.4 * radius), 5) > 1)
                            state = "UNTRACKING";
                    // Track slider if press keys AND move inside of sliderB
                    if (state === "UNTRACKING")
                    ...
                    //         state,
                    //         currentInput,
                    //         sliderPart: sliderParts[sliderPartIdx],
                    //         distance: Fixed(Dist(currentInput, sliderParts[sliderPartIdx]) / radius, 5),
                    //     });
                }
