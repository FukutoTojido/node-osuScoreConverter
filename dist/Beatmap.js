...
class Beatmap {
    ...
    static maxCombo;
    static baseData;
    static modMultiplier;
    static difficultyMultiplier;
    static stackOffset;
    static hitWindows;
    mods;
    ...
    buildMapData() {
        ...
        const drainTime = (Beatmap.baseData.hitobjects.at(-1).time -
            (Beatmap.baseData.events.breakPeriods.reduce((accumulated, curr) => accumulated + curr.length, 0) +
                Beatmap.baseData.hitobjects[0].time)) /
            1000;
    ...
    calculateMaximumScore() {
        if (!Beatmap.baseData)
            return;
        Beatmap.maxScore = Beatmap.baseData.hitobjects.reduce((accumulated, curr) => {
            const ret = accumulated + curr.calculateScore(300);
            Beatmap.the += Math.max(Beatmap.maxCombo - (curr instanceof Slider ? 1 : 2), 0);
            return ret;
        }, 0);
    }
    testAlgo() {
        Beatmap.maxScore_V2 =
            Beatmap.baseData.hitobjects.length * 300 + ((Beatmap.difficultyMultiplier * Beatmap.modMultiplier) / 25) * 300 * Beatmap.the;
        // console.table({ V2: Beatmap.maxScore_V2, V1: Beatmap.maxScore });
    }
}
